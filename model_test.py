...
class PretrainModelManager:
    def __init__(self, args, data):
        ...
        self.num_training_steps = int(
            len(data.train_labeled_examples) / args.train_batch_size) * args.num_train_epochs
    ...
    def get_optimizer(self, args):
        ...
        optimizer = AdamW(optimizer_grouped_parameters, lr=args.lr_pre, correct_bias=False)
        return optimizer
...
class BertForModel(nn.Module):
    ...
    def forward(self, input_ids = None, token_type_ids = None, attention_mask=None , labels = None, mode = None, centroids = None, labeled = False, feature_ext = False):
        ...
        _, pred = torch.max(logits_softmax, 1)

        if feature_ext:
            return pooled_output, pred, logits_softmax
        elif mode == 'train':
            loss = nn.CrossEntropyLoss()(logits, labels)
            return loss, logits
        elif mode == 'soft_train':
            loss = self.SoftCrossEntropy(logits, labels)
            return loss, logits
        else:
